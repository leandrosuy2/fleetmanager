...
class HomeController extends Controller {
	...
	public function index() {
		...
		$data['vehicle_count'] = VehicleModel::where('in_service', 1)->count();
		$data['vehicle'] = VehicleModel::where('in_service', 1)->latest('id')->take(18)->get();
		...
		else
		{
			if (Login::attempt(['email' => $request->email, 'password' => $request->password])) {
				...
				if ($user->user_type == "C") {
					...
					$user->save();
					// return redirect('/');
					return response()->json(['status' => 100, 'redirect' => url('/')]);
				} elseif (in_array($user->user_type, ["S", "O"], true)) {
					$user->login_status = 1;
					$user->save();
					return response()->json(['status' => 100, 'redirect' => url('/admin')]);
				} else {
					Auth::logout();
